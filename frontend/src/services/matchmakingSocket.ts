...
class MatchmakingSocket {
  ...
  connect(): Socket {
    ...
    // --- LIFECYCLE EVENTS ---
    this.socket.on('connect', () => console.log(`🟢 [Matchmaking] Connected! Socket ID: ${this.socket?.id}`));
    this.socket.on('disconnect', (reason) => console.warn(`🔴 [Matchmaking] Disconnected. Reason: ${reason}`));
