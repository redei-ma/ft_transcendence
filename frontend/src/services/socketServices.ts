...
export class SocketService {
    ...
    connect(url: string, userDbId?: string): Socket {
        ...
        // --- LIFECYCLE EVENTS ---
        this.socket.on('connect', () => console.log(`🟢 [GameSocket] Connected! Socket ID: ${this.socket?.id}`));
        this.socket.on('disconnect', (reason) => console.warn(`🔴 [GameSocket] Disconnected. Reason: ${reason}`));
        
        // --- RECONNECTION EVENTS ---
        this.socket.on('reconnect_attempt', (attempt) => console.log(`🔄 [GameSocket] Reconnect attempt #${attempt}...`));
        this.socket.on('reconnect', (attempt) => console.log(`✅ [GameSocket] Reconnected successfully after ${attempt} attempts`));
        this.socket.on('reconnect_error', (error) => console.error(`❌ [GameSocket] Reconnect error:`, error.message));
        this.socket.on('reconnect_failed', () => console.error(`💀 [GameSocket] Reconnection totally failed.`));
