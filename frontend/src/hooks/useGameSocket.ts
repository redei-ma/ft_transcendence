...
//       const players: PlayerSnapshot[] = payload.entities?.players || [];
//       const bullets: BulletSnapshot[] = payload.entities?.bullets || [];

//       setGameState({ timestamp: Date.now(), players, bullets });
//     };

//     const handleGameOver = (data: GameOverPayload) => {
//       log.game('Game over:', data);
//       setGameOver(data);
//     };
...
export function useGameSocket(): UseGameSocketReturn {
  ...
  useEffect(() => {
    ...
    const handleGameState = (payload: any) => {
      ...
      // { entities: { players: PlayerSnapshot[], bullets: BulletSnapshot[] }, time: number }
