...
export function ImpactEffect({ snapshot }: ImpactEffectProps) {
  ...
  useFrame((_, delta) => {
    ...
    // Flash
    if (flashRef.current) {
      flashRef.current.scale.setScalar(0.5 + progress * 3);
    ...
    // Zeus: fulmini esplosivi
    for (const line of boltObjects) {
      const posAttr = line.geometry.getAttribute('position') as THREE.BufferAttribute;
      const radius = (0.5 + progress * 3) * 1.5;
      ...
      for (let j = 0; j < BOLT_SEGMENTS; j++) {
        ...
        const jit = Math.sin(t * Math.PI) * 0.3 * (1 + progress);
