...
export function ZeusAura({ playerId }: ZeusAuraProps) {
  ...
  useFrame((_, delta) => {
    ...
    // Lettura live dallo store — nessun re-render React
  ...
  const innerOpacity = 0.06;

  return (
    <group ref={groupRef} position={[0, 2.4, 0]}>
      <mesh>
        <sphereGeometry args={[0.9, 16, 16]} />
        <meshBasicMaterial color={new THREE.Color(0, 4, 10)} transparent opacity={coreOpacity} toneMapped={false} />
      </mesh>

      <mesh>
        <sphereGeometry args={[3.0, 24, 24]} />
        <meshBasicMaterial color={new THREE.Color(0, 0.5, 1)} transparent opacity={innerOpacity} toneMapped={false} side={THREE.DoubleSide} />
      </mesh>
