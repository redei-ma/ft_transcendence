...
//     uOpacity: { value: 0.35 }, // <-- Abbassato da 0.9 a 0.35
//     uRotation: { value: 0 }, uHeatBias: { value: 0.05 },
//   }), []);

//   const outerUniforms = useMemo(() => ({
//     uTime: { value: 0 }, uIntensity: { value: 1.2 }, uDeform: { value: 0.4 },
//     uOpacity: { value: 0.15 }, // <-- Abbassato da 0.3 a 0.15
//     uRotation: { value: 0 }, uHeatBias: { value: 0.05 },
//   }), []);
...
//           opacity={0.15}
//           toneMapped={false} 
//           depthWrite={false} //per evitare occlusioni
//         />
//       </mesh>
...
//           transparent 
//           opacity={0.05} // <-- Abbassato
//           toneMapped={false} 
//           depthWrite={false} // <-- Aggiunto
//         />
//       </mesh>
//       <mesh>
...
// Dimensioni basate rigidamente sul server
const BASE_AURA_RADIUS = GameConfig.PLAYER.RADIUS; // Moltiplicatore visivo per lasciare fuori la testa
const MELEE_AURA_RADIUS = GameConfig.COMBAT.MELEE_HITBOX_RADIUS;
...
export function AdeAura({ playerId }: AdeAuraProps) {
  ...
  const currentScale = useRef(BASE_AURA_RADIUS);
  const rotation = useRef(0);
  const time = useRef(0);
  const activeEmbers = useRef(EMBER_COUNT_IDLE);
  ...
  const embersSystem = useMemo(() => {
    ...
    // Dimensione scalata per evitare particelle giganti quando il gruppo si espande
    const material = new THREE.PointsMaterial({
      size: 0.06, color: new THREE.Color(6, 1.5, 0),
      transparent: true, opacity: 0.7, toneMapped: false,
      blending: THREE.AdditiveBlending, depthWrite: false,
    });
    const points = new THREE.Points(geometry, material);
  ...
  useFrame((_, delta) => {
    ...
    // 1. DINAMICA DELLA SCALA
    let targetRadius = BASE_AURA_RADIUS;
    if (isDefending) targetRadius = BASE_AURA_RADIUS * 0.85;
    else if (isMelee) targetRadius = MELEE_AURA_RADIUS;

    // Interpolazione sincronizzata con il server
    const lerpFactor = GameConfig.RENDERING.INTERPOLATION_SPEED * (dt * 60);
    currentScale.current = THREE.MathUtils.lerp(currentScale.current, targetRadius, lerpFactor);
    groupRef.current.scale.setScalar(currentScale.current);

    // 2. DINAMICA DELLA ROTAZIONE
    const rotSpeed = isDefending ? 0.4 : isMelee ? 3.5 : 1.0;
    rotation.current += rotSpeed * dt;
    groupRef.current.rotation.y = rotation.current;

    // 3. DINAMICA PARTICELLE
    const targetEmbers = isDefending ? 30 : isMelee ? EMBER_COUNT_ATTACK : EMBER_COUNT_IDLE;
    activeEmbers.current = Math.round(THREE.MathUtils.lerp(activeEmbers.current, targetEmbers, 1 - Math.pow(0.01, dt)));
  ...
  return (
    // Centrato usando il raggio del giocatore dal GameConfig
    <group ref={groupRef} position={[0, GameConfig.PLAYER.RADIUS, 0]}>
      <mesh>
        <sphereGeometry args={[0.75, 16, 16]} />
        <meshBasicMaterial color={new THREE.Color(8, 2, 0)} transparent opacity={0.15} toneMapped={false} depthWrite={false} />
      </mesh>
      <mesh>
        <sphereGeometry args={[1.5, 16, 16]} />
        <meshBasicMaterial color={new THREE.Color(3, 0.3, 0)} transparent opacity={0.05} toneMapped={false} depthWrite={false} />
      </mesh>
      <mesh>
...
function respawnEmber(pos: Float32Array, vel: Float32Array, life: Float32Array, maxLife: Float32Array, i: number) {
  const theta = Math.random() * Math.PI * 2;
  const phi = Math.random() * Math.PI;
  // Raggio normalizzato a 1, poiché la dimensione finale è data dal groupRef.scale
  const r = 1.0 * (0.8 + Math.random() * 0.4); 
  pos[i*3] = r*Math.sin(phi)*Math.cos(theta);
  pos[i*3+1] = r*Math.cos(phi);
  pos[i*3+2] = r*Math.sin(phi)*Math.sin(theta);
  vel[i*3] = (Math.random()-0.5)*2.0;
