...
// import { CharacterName, MatchMode, GameConfig, Pillar } from '@transcendence/types';
...
// type ServerWall  = { id: string; position: { x: number; z: number }; width: number; depth: number };
// type CustomWall  = { id: string; x: number; z: number; width: number; depth: number };
// type WallEntry   = ServerWall | CustomWall;

// type CustomPillar  = { id: string; x: number; z: number; radius: number };
// type PillarEntry   = Pillar | CustomPillar;
...
//         {[...(world?.map?.walls || []), ...customWalls].map((w: WallEntry, i: number) => {
...
//         {[...(world?.map?.pillars || []), ...customPillars].map((p: PillarEntry, i: number) => {
