...
export default function FriendsSidebar({
  ...
  const activeInvites = gameInvites.filter(
  ...
  const onlineCount = friends.filter(
    (f) =>
      ...
      f.friend.status === 'IN_GAME' ||
      f.friend.status === 'IN_QUEUE',
  ).length;
  ...
  const handleAddFriend = async () => {
    ...
    if (input.length < USERNAME_MIN)
      return setAddError(
        `Username too short (min ${USERNAME_MIN} characters).`,
      );
    if (input.length > USERNAME_MAX)
      return setAddError(`Username too long (max ${USERNAME_MAX} characters).`);
  ...
  const handleRejectInvite = (invite: api.GameInvite) =>
            ...
            (sortedFriends.length === 0 ? (
              <div style={{ padding: '32px 16px', textAlign: 'center' }}>
                ...
                >
                  No friends yet.
                  <br />
                  Use the Add tab to send a request!
                </p>
              </div>
            ) : (
              sortedFriends.map((f) => {
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div
                        style={{
                          fontSize: '10px',
                          color: statusColor(f.friend.status),
                        }}
                      >
                    ...
                    >
                      {isOnline &&
                        (inviteMsg[f.friend.id] ? (
                          ...
                          <button
                            onClick={() => handleSendInvite(f.friend.id)}
                      ...
                      <button
                        ...
                        onMouseEnter={(e) => {
                          ...
                          e.currentTarget.style.color = theme.colors.dead;
                        }}
                        onMouseLeave={(e) => {
              ...
              {activeInvites.length > 0 && (
                <>
                  ...
                  {activeInvites.map((inv) => {
                    ...
                    const ttlTotal = Math.max(
                      1,
                      Math.ceil(
                        (expiresAt - new Date(inv.createdAt).getTime()) / 1000,
                      ),
                    );
              ...
              {!requests?.received || requests.received.length === 0 ? (
                ...
                requests.received.map((r) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleAcceptFriend(r.friend.id)}
                      style={{
                        padding: '4px 10px',
                        background: theme.colors.hpHigh,
                        border: 'none',
                        borderRadius: '2px',
                        color: theme.colors.bgDark,
                        fontFamily: theme.fonts.heading,
                    ...
                    <button
                      onClick={() => handleRejectFriend(r.friend.id)}
                      style={{
                        padding: '4px 8px',
                        background: 'none',
              ...
              {!requests?.sent || requests.sent.length === 0 ? (
                ...
                requests.sent.map((r) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleRemove(r.friend.id)}
                      style={{
                        background: 'none',
                        border: 'none',
                        color: theme.colors.textMuted,
          ...
          {!loading && tab === 'add' && (
            <div style={{ padding: '16px' }}>
              ...
              >
                ...
                >
                  Your Username is visible on your profile page. Share it with
                  friends!
                </p>
              </div>
            </div>
