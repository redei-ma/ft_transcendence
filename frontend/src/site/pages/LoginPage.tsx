...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  // RESEND VERIFICATION - Riscritto per essere a prova di bomba
  const handleResend = async () => {
    setError("");
    setSuccessMsg("");
    ...
    // Ricerca a cascata dell'email corretta
    ...
    // Se non abbiamo trovato nulla che assomigli a un'email (es. ha inserito solo l'username)
    if (!targetEmail) {
      ...
      <div style={{
        ...
        {successMsg && (
          <div style={{ width: "100%", padding: "8px 12px", marginBottom: "12px", background: "rgba(168,198,108,0.15)", border: `1px solid ${theme.colors.hpHigh}`, borderRadius: "2px", color: theme.colors.hpHigh, fontFamily: theme.fonts.mono, fontSize: "12px", textAlign: "center" }}>
            ...
            {(successMsg.includes("Registrazione completata") || successMsg.includes("reinviata a")) && (
              <button onClick={handleResend} style={{ display: 'block', margin: '8px auto 0', padding: '4px 8px', background: 'none', border: `1px solid ${theme.colors.hpHigh}`, color: theme.colors.hpHigh, cursor: 'pointer', fontSize: '11px', borderRadius: '2px' }}>
                Re-invia email
              </button>
            )}
          </div>
        ...
        {error && (
          <div style={{ width: "100%", padding: "8px 12px", marginBottom: "12px", background: "rgba(232,64,87,0.15)", border: `1px solid ${theme.colors.dead}`, borderRadius: "2px", color: theme.colors.dead, fontFamily: theme.fonts.mono, fontSize: "12px", textAlign: "center" }}>
            {error}
            {error.toLowerCase().includes("verify your email") && (
              <button onClick={handleResend} style={{ display: 'block', margin: '8px auto 0', padding: '4px 8px', background: 'none', border: `1px solid ${theme.colors.dead}`, color: theme.colors.textPrimary, cursor: 'pointer', fontSize: '11px' }}>
                Re-invia email
              </button>
            )}
          </div>
