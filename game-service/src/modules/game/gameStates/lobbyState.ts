...
export class LobbyState implements IGameState{
	...
	addPlayer(player: MatchMakingData, socketId: string | undefined): ExitStatus{
		...
		if (spawnIndex == -1){
			...
			return {status: ErrorCode.MATCH_ALREADY_STARTED, message: 'The match is already started, please search another game'};
		}

		const newPlayer: Player = getNewPlayer(this.session.gameWorld,
			socketId, spawnIndex,
			player.characterName, player.userDbId,
			entityId, player.isAiPlayer, this.session.getPlayerIndex(), this.session.matchType);
